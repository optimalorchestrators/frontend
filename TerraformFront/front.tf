...
resource "null_resource" "amplify_deployment" { 
  ...
  provisioner "local-exec" { # permet d'indiquer que la commande ci-dessous sera exécutée en local, sur la machine exécutant le Terraform
    command = "AWS_ACCESS_KEY_ID=${var.aws_access_key_id} AWS_SECRET_ACCESS_KEY=${var.aws_secret_access_key} aws amplify start-job --app-id ${aws_amplify_app.morningnewstestfront.id} --branch-name main --job-type RELEASE"
  }
}
